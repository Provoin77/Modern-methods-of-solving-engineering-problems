...
- **Онтология отвечает на вопрос:** «Какие слябы являются рисковыми с точки зрения их технологического маршрута и основных дефектов?». То есть для любого конкретного сляба она позволяет автоматически определить, относится ли он к классу `РисковыйСляб` при заданном маршруте (с критичным оборудованием) и наборе дефектов (с критичным дефектом).
- **Типы отношений (≥2):**
...
- `task3_1_ontology.ttl` — экспорт онтологии без SWRL-правил (OWL + транзитивные и функциональные свойства).
- `task3_2_ontology.ttl` — экспорт онтологии с добавленным SWRL-правилом классификации `РисковыйСляб`.
...
- Открыть проект в **Protege** → при необходимости отредактировать классы, свойства, индивиды или SWRL-правила →  
  **File → Save as… → Turtle Syntax (.ttl)** для получения актуальной версии `task3_1_ontology.ttl` и `task3_2_ontology.ttl`.
